...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const userFirstName = firstName || email.split('@')[0] || 'User';
  
  const actualBusinessName = businessName || 'FieldServ Ai';
...
function generateWelcomeEmailTemplate({
  ...
  const companyDisplayName = businessName || 'FieldServ Ai';
  const actionUrl = `https://www.leadprospecting.ai/setup-password?token=${setupToken}`;
  
  const subject = `Welcome to FieldServ Ai, ${userFirstName}! 🎉`;
  
  const html = `
    <!--[if mso]>
    <style type="text/css">
      .fallback-text { display: block !important; }
    </style>
    <![endif]-->
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; background: #ffffff;">
      ...
      <div style="padding: 40px 30px; background: white; border-radius: 0 0 12px 12px; box-shadow: 0 4px 20px rgba(0,0,0,0.1);">
        ...
        <div style="background: linear-gradient(135deg, #FFF8F5 0%, #FFF0E8 100%); padding: 30px; border-radius: 12px; text-align: center; margin: 30px 0; border: 1px solid #FFE4D6;">
          <h3 style="color: #333; margin: 0 0 15px 0; font-size: 20px;">🔐 Set Up Your Password</h3>
          <p style="color: #666; margin-bottom: 25px; font-size: 14px;">Click below to create your secure password and access your dashboard</p>
          ...
          <p style="color: #888; font-size: 12px; margin: 20px 0 0 0;">🔒 This link expires in 24 hours</p>
        </div>

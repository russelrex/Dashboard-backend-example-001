// pages/api/oauth/callback-private.ts
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Use original marketplace credentials for private callback
    const appCredentials = {
      client_id: process.env.GHL_MARKETPLACE_CLIENT_ID,
      client_secret: process.env.GHL_MARKETPLACE_CLIENT_SECRET
    };
    console.log('[OAuth Callback Private] Using original marketplace credentials');
    
    console.log('[OAuth Callback Private] Referer:', referer);
    console.log('[OAuth Callback Private] Is marketplace install:', isMarketplaceInstall);
    console.log('[OAuth Callback Private] Using app credentials for:', appCredentials.client_id);

    // Exchange code for tokens
    console.log('[OAuth Callback Private] Exchanging code for tokens...');
    ...
    const tokenResponse = await axios.post(
      ...
      new URLSearchParams({
        ...
        code: code as string,
        redirect_uri: `${process.env.NEXT_PUBLIC_API_URL || 'https://lpai-backend-omega.vercel.app'}/api/oauth/callback-private`
      }),
      {
        headers: {
    ...
    try {
      ...
      if (userType === 'Company' && !tokenLocationId) {
        ...
        if (targetLocationId) {
          ...
          console.log('[OAuth Callback Private] Skipping location token fetch - will be handled by background queue');
          ...
          await db.collection('install_retry_queue').insertOne({
            ...
            reason: 'oauth_callback_private_company_install',
            attempts: 0,
            status: 'pending',
            createdAt: new Date(),
          ...
          console.log(`[OAuth Callback Private] Location ${targetLocationId} added to setup queue`);
          
          // Redirect to location-specific progress page
          const progressUrl = `https://leadprospecting.ai/progress/${targetLocationId}`;
          console.log('[OAuth Callback Private] Redirecting to location progress:', progressUrl);
...
async function processInstallationSetup(locationId: string, companyId: string) {
  ...
  try {
    ...
    // 3. Snapshot will be applied later in setup-location flow
    console.log(`[Installation Setup] Snapshot will be applied in setup-location flow`);

    // 4. Users will be synced later in setup-location.ts - removed duplicate sync
...
async function createGHLObjectsAndCaptureIDs(locationId: string, location: any) {
  ...
  try {
    ...
    // 7. QUEUE CALENDAR ACTIVATION FOR AFTER USER SYNC
    // Standard calendars that need activation
    const standardCalendarNames = ['Quote Calendar', 'Field Work', 'Customer Walk-Thru'];
    const standardCalendars = calendars.filter(cal => 
      standardCalendarNames.some(name => 
        cal.name.toLowerCase().includes(name.toLowerCase().split(' ')[0])
      )
    );
    
    if (standardCalendars.length > 0) {
      console.log(`[GHL Objects] Queueing activation for ${standardCalendars.length} standard calendars`);
      
      await db.collection('install_retry_queue').insertOne({
        _id: new ObjectId(),
        type: 'ACTIVATE_CALENDARS',
        locationId,
        payload: {
          calendarIds: standardCalendars.map(c => c.id),
          calendarNames: standardCalendars.map(c => c.name)
        },
        status: 'pending',
        scheduledFor: new Date(Date.now() + 90000), // 90 seconds after install starts (after user sync)
        attempts: 0,
        createdAt: new Date(),
        reason: 'post_snapshot_activation'
      });
      
      console.log(`[GHL Objects] Calendar activation queued for 90 seconds from now`);
    }
